...
/// <summary>
/// JY5500 continuous non-surround mode digital output
/// Author: JYTEK
...
namespace Winform_DO_ContinuousNoWrapping_Write_Digital_Trigger
{
    public partial class MainForm : Form
    {
        ...
        private void button_stop_Click(object sender, EventArgs e)
        {
            try
            {
                if (dotask != null) //Determine if the task exists
                {
                    //Stop Task 
                    dotask.Stop();
                }
            ...
            dotask.Channels.Clear();//Clear the channel that was added last time

            //重新StartParameter configuration按钮
            timer_FetchData.Enabled = false;
        ...
        /// <summary>
        /// Timer, write data to buffer to the local buffer every 100ms
        /// </summary>
        private void timer_FetchData_Tick(object sender, EventArgs e)
        {
            ...
            try
            {
                //Write data to buffer if the number of write points in the buffer is greater than or equal to the length of Write data to buffer
                if (dotask.AvaliableLenInSamples >= writeValue.GetLength(0))
                {
                    dotask.WriteData(writeValue, (uint)writeValue.GetLength(0), -1);
                }
            }
