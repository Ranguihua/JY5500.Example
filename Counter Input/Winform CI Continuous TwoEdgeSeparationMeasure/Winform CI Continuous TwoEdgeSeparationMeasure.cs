...
/// <summary>
/// JY5500 continuous mode TwoEdgeSeparation detection
/// Author: JYTEK
...
/// the number of Timebase pulses between the first signal and the rising edge of the second
/// signal is written to the register.Measure two edge differences that are not on the same 
/// channel The second is to enable implicit clocking: when a measurement is completed, the 
/// count value is written to the register.
/// 2. Select which two rising edges are to be measured. The first signal comes from the Gate
/// side and the second signal comes from the Aux side.
///
/// TwoEdgeSeparation detection:
/// In this mode, the difference between the two channel edges of CTR0_GATE and CTR0_AUX is 
/// mainly recorded in this mode.
/// </summary>

namespace Winform_CI_Continuous_TwoEdgeSeparationMeasure
{
